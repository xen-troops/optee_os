...
CC$(sm)		:= $(ccache-cmd)$(OPTEE_COMPILER_PATH)/clang --target=$(clang-target)
# Due to the absence of clang-cpp in AOSP's prebuilt version of clang,
# use the equivalent command of 'clang -E'
CPP$(sm)	:= $(ccache-cmd)$(OPTEE_COMPILER_PATH)/clang --target=$(clang-target) -E
LD$(sm)		:= $(ccache-cmd)$(OPTEE_COMPILER_PATH)/ld.lld

AR$(sm)		:= $(ccache-cmd)$(OPTEE_COMPILER_PATH)/llvm-ar
NM$(sm)		:= $(OPTEE_COMPILER_PATH)/llvm-nm
OBJCOPY$(sm)	:= $(OPTEE_COMPILER_PATH)/llvm-objcopy
OBJDUMP$(sm)	:= $(OPTEE_COMPILER_PATH)/llvm-objdump
READELF$(sm)	:= $(OPTEE_COMPILER_PATH)/llvm-readelf
